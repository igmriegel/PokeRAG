...
run_step "Pytest unit + integration" env PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 python -m pytest -p pytest_cov -p asyncio -p no:rerunfailures -p no:pytest_rerunfailures -p no:langsmith_plugin -p no:langsmith -p no:xdist -p no:xdist.looponfail tests/unit/ tests/integration/
run_step "Pytest coverage gate" env PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 python -m pytest -p pytest_cov -p asyncio -p no:rerunfailures -p no:pytest_rerunfailures -p no:langsmith_plugin -p no:langsmith -p no:xdist -p no:xdist.looponfail tests/unit/ tests/integration/ --cov=pokemon_tcg_rag --cov-fail-under=90 --cov-report=xml
