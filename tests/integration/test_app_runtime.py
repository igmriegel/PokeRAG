...
def test_build_runtime_container_uses_offline_fallback_without_openai(
    monkeypatch,
) -> None:
    ...
    try:
        ...
        assert (
            container.dense_retriever.retrieve(
                "Rare Candy",
                top_k=5,
...
def test_build_runtime_container_requires_openai_in_production(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        api_runtime, "load_corpus_manifest", lambda *_args, **_kwargs: dummy_manifest
    )
    monkeypatch.setattr(
        api_runtime, "load_chunks", lambda *_args, **_kwargs: [dummy_chunk]
    )
